...
| Native prompts, pickers, notifications, reveal/open-with | `NeedsRuntime` | ArkUI owns prompts. The bridge retains each GPUI answer index, uses the system's adaptive button layout and standard style, and gives Cancel default keyboard focus when it exists. The newest package needs a repeat removal, warning, and multi-answer runtime pass. |
