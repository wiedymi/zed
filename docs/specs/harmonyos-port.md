...
| App menu/system bridges | In-window Zed/File/Edit/Selection/View/Go/Run/Help menus use production actions. View includes the Agent and Collaboration panel actions. Settings and later GPUI windows use non-modal ArkUI subwindows with independent XComponents, surfaces, atlases, input, and close lifecycle. ArkUI attachment now makes the auxiliary GPUI window active before it accepts input. The system title-bar close event goes through GPUI before ArkUI destroys the subwindow. GPUI applies `should_close`, releases the view, destroys the native subwindow, and restores the previous active window. A failed resize no longer requeues itself after ArkUI destroys an auxiliary surface; the backend marks that surface unavailable until a new native lifecycle event recreates it. The app creates its configuration directory before Settings or a language server uses it. ArkUI applies the requested auxiliary size and position before show. OHOS uses explicit application quit. Native prompts retain GPUI answer indices and use ArkUI's adaptive layout and standard button style; Cancel receives default keyboard focus when present. The icon uses the real Zed mark with a larger safe area. These changes still need runtime checks. |
...
| Agent panel | `NeedsRuntime` | The action is registered before asynchronous panel restore. It creates a missing panel before it gives it focus. OHOS reveals the real loaded panel after every dock has finished restoring. Test startup, View > Agent Panel, a new workspace, and a restored workspace. |
...
| Native prompts, pickers, notifications, reveal/open-with | `NeedsRuntime` | OHOS keeps GPUI's platform prompt builder instead of installing Zed's in-app prompt renderer, so ArkUI owns Trash, Delete, Remove, warning, and other confirmations. The bridge retains each GPUI answer index, uses the system's adaptive button layout and standard style, and gives Cancel default keyboard focus when it exists. The newest package needs a repeat removal, warning, and multi-answer runtime pass. |
...
GPUI state before asking ArkTS to destroy its subwindow. ArkUI intercepts a
native title-bar close request and first runs GPUI's `should_close`/`close`
callbacks. GPUI then destroys the subwindow and restores the prior active
window, so ArkUI and GPUI cannot retain different active-window state.
...
- [x] Package OpenSSH 10.4p1 clients with static OpenSSL, packaged native askpass, one bounded OHOS local identity adapter for client and shared UID lookups, a control-socket rename for the no-hard-link sandbox, and absolute-path SSH/SCP/SFTP/Git integration. The rebuilt package installs; a real connection still needs a runtime check.
